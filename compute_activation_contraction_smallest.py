...
#import resnet20_cifar_ac
import resnet10_cifar_ac
import resnet20_cifar_ac
...
if __name__=='__main__':
    ...
    args.pretrained = './log/cifar100/pgd-linf-eps8iter50-resnet20w4'
    #args.pretrained = './log/cifar100/pgd-linf-eps8iter50-resnet20w4'
    args.batch_size = 10
    args.limit_test = 100
    ...
    if args.arch == 'resnet20':
        pretrained_model = resnet20_cifar_ac.Model(args)
    elif args.arch == 'resnet10':
        pretrained_model = resnet10_cifar_ac.Model(args)
    else: 
    ...
    for i, (input, target) in enumerate(testloader):
        ...
        for key, value in output.items():
            if i == 0:
                activation_contraction[key] = output[key].detach().cpu().numpy()
            else:
                activation_contraction[key] = np.append(activation_contraction[key], output[key].detach().cpu().numpy())

        #pdb.set_trace()
        if (i+1)%args.limit_test == 0:
            break
    for key, value in activation_contraction.items():
        print(np.max(value))
    
    #pdb.set_trace()

    np.save(os.path.join(save_path, 'activation_contraction.npy'.format(args.batch_size*args.limit_test)), activation_contraction, allow_pickle=True)
