...
if __name__=='__main__':
    ...
    for i, (input, target) in enumerate(testloader):
        ...
        for key, value in digital_output.items():
            #err_batch[key] = mvm_output[key]-digital_output[key]
            #if i == 0:
            ...
            err_batch[key] = mvm_output[key] - digital_output[key] # the order is important for recording the mean difference between mvm and digital implementations
    ...
    logging.info("Gaussian noise mean and std to use for NVM crossbar model:{}, pretrained_model:{}".format(args.mvm_type, args.pretrained))

    # Create 2 dictionaries, one for mean and one for std
    dig_vs_mvm_mean = {}
    dig_vs_mvm_std = {}
    ...
    for key, value in dig_vs_mvm.items():
        mean_ = np.mean(value)
        dig_vs_mvm_mean[key] = mean_
        logging.info('{} {}'.format(key, mean_))
    ...
    # Saving Std information
    #logging.info('Saving Std Dictionaries')
    ...
    #   logging.info('Saved results dig_vs_mvm_std_N{}.npy to ==> {}'.format(args.batch_size*args.limit_test, save_path))

    # Saving Mean information
    logging.info('Saving Mean Dictionaries')
    if os.path.exists(os.path.join(save_path, 'dig_vs_mvm_mean_N{}.npy'.format(args.batch_size*args.limit_test))):
        os.remove(os.path.join(save_path, 'dig_vs_mvm_mean_N{}.npy'.format(args.batch_size*args.limit_test)))
        logging.info("Old File Removed")
    else:
        logging.info("The file did not exist before")
        np.save(os.path.join(save_path, 'dig_vs_mvm_mean_N{}.npy'.format(args.batch_size*args.limit_test)), dig_vs_mvm_mean, allow_pickle=True)
        logging.info('Saved results dig_vs_mvm_mean_N{}.npy to ==> {}'.format(args.batch_size*args.limit_test, save_path))
