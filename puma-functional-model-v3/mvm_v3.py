...
def float_to_16bits_tensor_fast(
    input, frac_bits, 
    bit_slice, bit_slice_num, input_bits
): # input is batch x n tensor / output is n x 16 tensor..
...
def mvm_tensor(zeros, shift_add_bit_stream, shift_add_bit_slice, output_reg, flatten_input, flatten_input_sign, bias_addr, 
    ...
    if bit_stream == 1:
        ...
        for i in range(bit_stream_num): # 16bit input
            input_stream = input_split[:,:,:,:,-1-i].reshape(
                (2, batch_size, xbars_row, 1, XBAR_ROW_SIZE, 1)
            ) #input is arranged from MSB---->LSB
            #####
            output_analog = torch.mul(xbars, input_stream)
            ...
            output_reg[:,:,:,:,i,:] = torch.sum(
                torch.mul(output_analog, shift_add_bit_slice), 5
            ) # -1 # adding across bit sliced dimension
...
def mvm_tensor_nonid(
    zeros, 
    shift_add_bit_stream, shift_add_bit_slice, 
    output_reg, output_analog, 
    Goffmat, G_real_flatten, G_real, 
    Xbar_model, Xbar_params,
    flatten_input,flatten_input_sign, 
    bias_addr, xbars, 
    bit_slice, bit_stream, 
    weight_bits, weight_bit_frac, 
    input_bits, input_bit_frac, adc_bit, acm_bit, 
    acm_bit_frac
):  #### These should be 'almost' completely changed. 
    ...
    inmax_V = Vmax
    inmin_V = 0
    inmax_test = Xbar_params['inmax_test']
    inmin_test = Xbar_params['inmin_test']
    ...
    if bit_stream == 1:
        for i in range(bit_stream_num): # 16bit input
            V_real_loop = V_real[:,:,:,-1-i].reshape(
                (batch_size, xbars_row, 1, XBAR_ROW_SIZE, 1)
            )   #V_real.shape batchsize, xbar_rows, xbarsize, num_bitstreams
            V_real_scaled_loop = V_real_scaled[:,:,:,-1-i].reshape(
                (batch_size, xbars_row, 1, XBAR_ROW_SIZE, 1)
            )

            output_bias_all = torch.sum(torch.mul(Goffmat,V_real_loop),3).unsqueeze(3).expand(
                batch_size, xbars_row, 1, XBAR_COL_SIZE, 1
            )
        ...
        for i in range(bit_stream_num): # 16bit input
            ...
            output_bias_all = torch.sum(torch.mul(Goffmat,V_real_loop),4).unsqueeze(4).expand(
                2,batch_size, xbars_row, 1, XBAR_ROW_SIZE, 1
            )#.to(device)
