...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_custom_norm:
            ...
            out = x
        #---- Layer 0
        act = {'conv0':{}, 'conv111':{}, 'conv112':{}, 'conv121':{}, 'conv122':{}, 'conv131':{}, 'conv132':{}, 'conv211':{}, 'conv212':{}, 'conv221':{}, 'conv222':{}, 'conv231':{}, 'conv232':{}, 'conv311':{}, 'conv312':{}, 'conv321':{}, 'conv322':{}, 'conv331':{}, 'conv332':{}, 'linear':{}}

        if self.store_act: act['conv0']['in'] = out.clone()
        out = self.conv0(out)
        if self.store_act: act['conv0']['out'] = out.clone()
        out = self.bn0(out)
        out = self.relu0(out)
        #---- Group 1 (16out)
        #---- Block 1.1
        ...
        #---- Layer 1.1.1
        if self.store_act: act['conv111']['in'] = out.clone()
        out = self.conv111(out)
        if self.store_act: act['conv111']['out'] = out.clone()
        out = self.bn111(out)
        out = self.relu111(out)
        #---- Layer 1.1.2
        if self.store_act: act['conv112']['in'] = out.clone()
        out = self.conv112(out)
        if self.store_act: act['conv112']['out'] = out.clone()
        out = self.bn112(out)
        #---- add residual
        out+=residual
        out = self.relu11(out)
        ...
        residual = out.clone()
        #---- Layer 1.2.1
        if self.store_act: act['conv121']['in'] = out.clone()
        out = self.conv121(out)
        if self.store_act: act['conv121']['out'] = out.clone()
        out = self.bn121(out)
        out = self.relu121(out)
        #---- Layer 1.2.2
        if self.store_act: act['conv122']['in'] = out.clone()
        out = self.conv122(out)
        if self.store_act: act['conv122']['out'] = out.clone()
        out = self.bn122(out)
        #---- add residual
        out+=residual
        out = self.relu12(out)
        ...
        residual = out.clone()
        #---- Layer 1.3.1
        if self.store_act: act['conv131']['in'] = out.clone()
        out = self.conv131(out)
        if self.store_act: act['conv131']['out'] = out.clone()
        out = self.bn131(out)
        out = self.relu131(out)
        #---- Layer 1.3.2
        if self.store_act: act['conv132']['in'] = out.clone()
        out = self.conv132(out)
        if self.store_act: act['conv132']['out'] = out.clone()
        out = self.bn132(out)
        #---- add residual
        out+=residual
        out = self.relu13(out)
        ...
        #---- Layer 2.1.1
        if self.store_act: act['conv211']['in'] = out.clone()
        out = self.conv211(out)
        if self.store_act: act['conv211']['out'] = out.clone()
        out = self.bn211(out)
        out = self.relu211(out)
        #---- Layer 2.1.2
        if self.store_act: act['conv212']['in'] = out.clone()
        out = self.conv212(out)
        if self.store_act: act['conv212']['out'] = out.clone()
        out = self.bn212(out)
        #---- add residual
        out+=residual
        out = self.relu21(out)
        ...
        residual = out.clone() 
        #---- Layer 2.2.1 
        if self.store_act: act['conv221']['in'] = out.clone()
        out = self.conv221(out)
        if self.store_act: act['conv221']['out'] = out.clone()
        out = self.bn221(out)
        out = self.relu221(out)
        #---- Layer 2.2.2
        if self.store_act: act['conv222']['in'] = out.clone()
        out = self.conv222(out)
        if self.store_act: act['conv222']['out'] = out.clone()
        out = self.bn222(out)
        #---- add residual
        out+=residual
        out = self.relu22(out)
        ...
        residual = out.clone() 
        #---- Layer 2.3.1
        if self.store_act: act['conv231']['in'] = out.clone()
        out = self.conv231(out)
        if self.store_act: act['conv231']['out'] = out.clone()
        out = self.bn231(out)
        out = self.relu231(out)
        #---- Layer 2.3.2
        if self.store_act: act['conv232']['in'] = out.clone()
        out = self.conv232(out)
        if self.store_act: act['conv232']['out'] = out.clone()
        out = self.bn232(out)
        #---- add residual
        out+=residual
        out = self.relu23(out)
        ...
        #---- Layer 3.1.1
        if self.store_act: act['conv311']['in'] = out.clone()
        out = self.conv311(out)
        if self.store_act: act['conv311']['out'] = out.clone()
        out = self.bn311(out)
        out = self.relu311(out)
        #---- Layer 3.1.2
        if self.store_act: act['conv312']['in'] = out.clone()
        out = self.conv312(out)
        if self.store_act: act['conv312']['out'] = out.clone()
        out = self.bn312(out)
        #---- add residual
        out+=residual
        out = self.relu31(out)
        ...
        residual = out.clone() 
        #---- Layer 3.2.1 
        if self.store_act: act['conv321']['in'] = out.clone()
        out = self.conv321(out)
        if self.store_act: act['conv321']['out'] = out.clone()
        out = self.bn321(out)
        out = self.relu321(out)
        #---- Layer 3.2.2
        if self.store_act: act['conv322']['in'] = out.clone()
        out = self.conv322(out)
        if self.store_act: act['conv322']['out'] = out.clone()
        out = self.bn322(out)
        #---- add residual
        out+=residual
        out = self.relu32(out)
        ...
        residual = out.clone() 
        #---- Layer 3.3.1 
        if self.store_act: act['conv331']['in'] = out.clone()
        out = self.conv331(out)
        if self.store_act: act['conv331']['out'] = out.clone()
        out = self.bn331(out)
        out = self.relu331(out)
        #---- Layer 3.3.2
        if self.store_act: act['conv332']['in'] = out.clone()
        out = self.conv332(out)
        if self.store_act: act['conv332']['out'] = out.clone()
        out = self.bn332(out)
        #---- add residual
        out+=residual
        out = self.relu33(out)
        ...
        if self.store_act: act['linear']['in'] = out.clone()
        out = self.linear(out)
        if self.store_act: act['linear']['out'] = out.clone()
        if self.store_act:
            return act
        else:
            return out
