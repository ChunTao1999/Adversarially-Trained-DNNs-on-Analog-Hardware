...
class Model(nn.Module):
    ...
    def forward(self, x):
        if self.use_custom_norm:
            ...
            out = x
        #print('deboleena')
        act = {'conv0':{}, 'conv111':{}, 'conv112':{}, 'conv211':{}, 'conv212':{}, 'conv311':{}, 'conv312':{}, 'conv321':{}, 'conv322':{}, 'linear':{}}
        #---- Layer 0
        if self.store_act:
            act['conv0']['in'] = out.clone()
        out = self.conv0(out)
        if self.store_act:
            act['conv0']['out'] = out.clone()
        out = self.bn0(out)
        out = self.relu0(out)
        #---- Group 1 (16out)
        #---- Block 1.1
        ...
        #---- Layer 1.1.1
        if self.store_act:
            act['conv111']['in'] = out.clone()
        out = self.conv111(out)
        if self.store_act:
            act['conv111']['out'] = out.clone()
        out = self.bn111(out)
        out = self.relu111(out)
        #---- Layer 1.1.2
        if self.store_act:
            act['conv112']['in'] = out.clone()
        out = self.conv112(out)
        if self.store_act:
            act['conv112']['out'] = out.clone()
        out = self.bn112(out)
        #---- add residual
        out+=residual
        out = self.relu11(out)       
        ...
        #---- Layer 2.1.1
        if self.store_act:
            act['conv211']['in'] = out.clone()
        out = self.conv211(out)
        if self.store_act:
            act['conv211']['out'] = out.clone()
        out = self.bn211(out)
        out = self.relu211(out)
        #---- Layer 2.1.2
        if self.store_act:
            act['conv212']['in'] = out.clone()
        out = self.conv212(out)
        if self.store_act:
            act['conv212']['out'] = out.clone()
        out = self.bn212(out)
        #---- add residual
        out+=residual
        out = self.relu21(out)
        ...
        #---- Layer 3.1.1
        if self.store_act:
            act['conv311']['in'] = out.clone()
        out = self.conv311(out)
        if self.store_act:
            act['conv311']['out'] = out.clone()
        out = self.bn311(out)
        out = self.relu311(out)
        #---- Layer 3.1.2
        if self.store_act:
            act['conv312']['in'] = out.clone()
        out = self.conv312(out)
        if self.store_act:
            act['conv312']['out'] = out.clone()
        out = self.bn312(out)
        #---- add residual
        out+=residual
        out = self.relu31(out)
        ...
        #---- Layer 3.2.1 
        if self.store_act:
            act['conv321']['in'] = out.clone()
        out = self.conv321(out)
        if self.store_act:
            act['conv321']['out'] = out.clone()
        out = self.bn321(out)
        out = self.relu321(out)
        #---- Layer 3.2.2
        if self.store_act:
            act['conv322']['in'] = out.clone()
        out = self.conv322(out)
        if self.store_act:
            act['conv322']['out'] = out.clone()
        out = self.bn322(out)
        #---- add residual
        out+=residual
        out = self.relu32(out)
        ...
        if self.store_act:
            act['linear']['in'] = out.clone()
        out = self.linear(out)
    
        if self.store_act:
            act['linear']['out'] = out.clone()
            return act
        else:
            return out
