...
import resnet20_new_interlc_conv312_conv332
...
if __name__=='__main__':
    ...
    if args.arch == 'resnet20':
        pretrained_model = resnet20_new_interlc_conv312_conv332.Model(args)
    elif args.arch == 'resnet10':
        pretrained_model = resnet10_new_interlc_conv212_conv322.Model(args)
    else: 
    ...
    save_path = os.path.join(args.pretrained, "clean", "interlayer_cushion_new")
    ...
    for i, (input, target) in enumerate(testloader):
        ...
        output_mid, residual, dim = model_eval.forward1(input_var)
    
        output_mid = output_mid.cuda()
        residual = residual.cuda()
        output = model_eval.forward2(output_mid, residual)
        jacobian = torch.autograd.functional.jacobian(model_eval.forward2,(output_mid, residual))
        input_norm = torch.norm(output_mid)
        jacobian_norm = torch.norm(jacobian[0])
        rhs_norm = torch.norm(jacobian[0]*output_mid)
        
        interlc = np.append(interlc, rhs_norm.item()*np.sqrt(dim[0]*dim[1])/(input_norm.item()*jacobian_norm.item()))
        #print(jacobian.size())  
        #pdb.set_trace()
    ...
    if os.path.exists(os.path.join(save_path, 'conv312_conv332.csv'.format(args.batch_size*args.limit_test))):
        os.remove(os.path.join(save_path, 'conv312_conv332.csv'.format(args.batch_size*args.limit_test)))
        print("Old File conv312_conv332.csv Removed")

    else:
        print("The file did not exist before")

    np.savetxt(os.path.join(save_path, 'conv312_conv332.csv'.format(args.batch_size*args.limit_test)), interlc, delimiter = ',')
    print("New file conv312_conv332.csv saved")

    print(args.pretrained)
    print("conv312_conv332")
