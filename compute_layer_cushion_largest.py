...
if __name__=='__main__':
    ...
    args.pretrained = './log/cifar100/clean-resnet20w4'
    #args.pretrained = './log/cifar100/pgd-linf-eps2iter50-resnet20w1'
    args.batch_size = 10
    args.limit_test = 100
    ...
    if args.arch == 'resnet20':
        pretrained_model = resnet20_cifar_lc.Model(args)
    elif args.arch == 'resnet10':
        pretrained_model = resnet10_cifar_lc.Model(args)
    else: 
    ...
    save_path = os.path.join(args.pretrained, "clean", "noise_stability")
    ...
    for i, (input, target) in enumerate(testloader):
        ...
        for key, value in output.items():
            if i == 0:
                layer_cushion[key] = output[key].detach().cpu().numpy()
            else:
                 layer_cushion[key] = np.append(layer_cushion[key], output[key].detach().cpu().numpy())
        #pdb.set_trace()
        if (i+1)%args.limit_test == 0:
            break
        
    for key, value in layer_cushion.items():
        # Use the minimum of all layer cushion values because we're looking for the largest value that satisfies all input examples
        #pdb.set_trace()
        print(np.min(value))
    
    #pdb.set_trace()

    np.save(os.path.join(save_path, 'layer_cushion.npy'.format(args.batch_size*args.limit_test)), layer_cushion, allow_pickle=True)
