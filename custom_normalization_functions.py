...
class custom_1channel_img_normalization_with_dataset_params(object):
    def __init__ (self, mean, std, img_dimensions, device='cpu'):
        self.mean = mean
        assert len(self.mean) == 1, 'Custom norm function is for 1 channel images. Expected 3 elements for mean, got {}'.format(len(mean))
        self.std  = std
        assert len(self.std) == 1, 'Custom norm function is for 1 channel images. Expected 3 elements for std, got {}'.format(len(std))
        ...
        self.mean_expanded = (self.mean[0]*torch.ones((1, self.img_dims[1], self.img_dims[2]))).to(self.in_device)
